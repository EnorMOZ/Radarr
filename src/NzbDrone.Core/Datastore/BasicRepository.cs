...
namespace NzbDrone.Core.Datastore
{
    public interface IBasicRepository<TModel> where TModel : ModelBase, new()
    {
        ...
        void SetFields(IList<TModel> model, params Expression<Func<TModel, object>>[] properties);
        TModel Single();
    ...
    public class BasicRepository<TModel> : IBasicRepository<TModel> where TModel : ModelBase, new()
    {
        ...
        public BasicRepository(IDatabase database, IEventAggregator eventAggregator)
        {
            ...
            var excluded = TableMapping.Mapper.ExcludeProperties(typeof(TModel)).Select(x => x.Name).ToList();
            excluded.Add(_keyProperty.Name);
            _properties = typeof(TModel).GetProperties().Where(x => !excluded.Contains(x.Name)).ToList();
        ...
        {
            var model = Query(Builder().WhereEqual<TModel>(x => x.Id, id)).FirstOrDefault();
        ...
        public IEnumerable<TModel> Get(IEnumerable<int> ids)
        {
            ...
            var result = Query(Builder().WhereIn<TModel>(x => x.Id, ids));
        ...
        {
            var sql = builder.AddTemplate(_deleteTemplate);
        ...
        {
            Delete(Builder().WhereEqual<TModel>(x => x.Id, model.Id));
        }

        public void Delete(int id)
        {
            Delete(Builder().WhereEqual<TModel>(x => x.Id, id));
        }

        public void DeleteMany(IEnumerable<int> ids)
        {
            Delete(Builder().WhereIn<TModel>(x => x.Id, ids));
        }

        public void DeleteMany(List<TModel> models)
        {
            Delete(Builder().WhereIn<TModel>(x => x.Id, models.Select(x => x.Id)));
        }
        ...
        protected virtual SqlBuilder GetPagedBuilder() => BuilderBase();
        protected virtual IEnumerable<TModel> GetPagedSelector(SqlBuilder.Template sql) => GetResults(sql);

        public virtual PagingSpec<TModel> GetPaged(PagingSpec<TModel> pagingSpec)
        {
            pagingSpec.Records = GetPagedRecords(GetPagedBuilder().SelectAll(), pagingSpec, GetPagedSelector);
            pagingSpec.TotalRecords = GetPagedRecordCount(GetPagedBuilder().SelectCount(), pagingSpec);
